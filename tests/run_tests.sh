#!/bin/bash
#
# Counter Tray Designer Test Runner
# Runs OpenSCAD test files, exports finalized STL, then renders orthographic
# PNG views from each.
#
# Usage:
#   ./run_tests.sh                       # Run all tests (iso+top views)
#   ./run_tests.sh test_backward_compat  # Run a single test
#   ./run_tests.sh --csg-only            # Only check compilation (fast, no PNG)
#   ./run_tests.sh --views iso,top,front # Render specific views
...
RENDER_SCAD="/tmp/ctd_render_views.scad"
...
usage() {
    ...
    echo "  --views LIST      Comma-separated views to render (default: iso,top)"
    ...
    echo "                    If none specified, runs all tests/scad/test_*.scad"
    echo ""
    echo "Pipeline: .scad -> CSG check -> STL export -> PNG renders (ortho+edges)"
    echo ""
    echo "Examples:"
    echo "  $0                                  # Full run, all tests"
    echo "  $0 test_backward_compat             # Single test"
    echo "  $0 --views iso,top,front test_*     # Specific views"
...
if ! command -v openscad &>/dev/null; then
    echo -e "${RED}FATAL${NC}: openscad not found in PATH."
    exit 1
fi

CSG_ONLY=false
REQUESTED_VIEWS="iso,top"
TESTS=()
