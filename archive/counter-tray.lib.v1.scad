...
module Make( TEMPLATE = TEST)
{
    ...
    num_counters_x = floor(( max_tray_dimensions.x - 2* tray_min_padding.x - 4* counter_margin.y ) / counter_outer_width );
    tray_inner_width = num_counters_x * counter_outer_width + counter_margin.x * 2;
  //  tray_padding_width =  tray_min_padding.x;//( tray_outer_width - tray_inner_width ) / 2;
    ...
    tray_outer_width_ideal = tray_inner_width + tray_min_padding.x * 2 + 1;


    num_counters_y = floor((max_tray_dimensions.y - 2* tray_min_padding.y - 4* counter_margin.y ) / counter_outer_height );
    tray_inner_height = num_counters_y * counter_outer_height + counter_margin.y * 2;
  //  tray_padding_height =  tray_min_padding.y;//( tray_outer_height - tray_inner_height ) / 2;
    ...
    tray_outer_height_ideal = tray_inner_height + 2 * tray_min_padding.y + 1;
