...
- Key files: `counter_tray_designer_lib.1.scad` (engine), `tray-library.scad` (game presets + invocation), `archive/counter-tray.lib.v1.scad` (legacy; refer only for history), `archive/dimensions.lib.scad` (old constants), `.3mf` files are exported prints.
