include <dimensions.lib.scad>

VERSION = "1.00";
COPYRIGHT_INFO = "\tCounter Tray\n\thttps://github.com/dppdppd/counter_tray\n\n\tCopyright 2024 Ido Magal\n\tCreative Commons - Attribution - Non-Commercial - Share Alike.\n\thttps://creativecommons.org/licenses/by-nc-sa/4.0/legalcode";
...
module Main()
{
    ...
    make_tray = find_value( DATA, G_MAKE_TRAY_B, default = false);
    make_lid = find_value( DATA, G_MAKE_LID_B, default = false);
    tray_min_padding = find_value( DATA, G_MIN_PADDING_XY, default = [8,8]);
    ...
    all_sets_y_offset = 
        ( tray_size_3d.y - get_total_y_size() )/2;
    ...
    function get_total_y_size( setidx = 0, sum = 0) =
        setidx < num_sets ?
        let(sum_new = is_enabled(setidx) ? sum + get_set_size( setidx ).y : sum)
        get_total_y_size( setidx + 1, sum_new) :
        sum;
...
///////////////////////////////////////////
    ...
    module MakeMagnetSlots( body_depth, is_lid = false )
    {
        ...
        g_magnet_distance_from_edge = [4,4];//[counter_size_outer.x/2,counter_size_outer.y/2];
        ...
        {
            translate([tray_size_3d.x/2, tray_size_3d.y/2, 0])
            translate([0, 0, 0])
            {
                PlaceInFourCorners()
    ...
    {
        if (make_tray)
        translate( [tray_size_3d.x + 10, 0, 0])
        children();
        else
        children();
    }
    ...
    module MakeSetCounterHoles( setidx )
    {
        ...
        module MakeCounterHole( extra = 0)
        {
            ...
            translate( [counter_size_outer.x/2, counter_size_outer.y/2, floor_thickness/2])

            if ( !round_counters)
            hull()
            {
                translate( [ -dist_from_center.x, -dist_from_center.y, 0] )
                cylinder( h=floor_thickness * 1, d=2.5, center=true );
            ...
            {  
                cylinder( h=floor_thickness * 1, d=counter_size.x - inset, center=true );           
            }
        }
    }
